...
3. 授权范围
   ...
   - 自动启动 Agent、等待扫描器和工具协作 AI 等选项属于运行策略，不等同于授权范围开关。
...
6. 工作模式
   - balanced：覆盖与速度平衡。
   - fast：缩短等待并优先快速资产结果。
   - deep：扩大检测覆盖并允许更多 Agent 批次。
   - cautious/manual：降低自动化强度或关闭自动 Agent。
7. 配置保存位置
...
2. 协调器从 results 和 tool_data 吸收新增结果并写入资产总线。
3. 资产总线维护资产代次、来源、安静窗口和就绪状态。
4. 达到启动条件后，协调器在共享 Windows Terminal 窗口中新增一个 CLI 执行批次。
5. 每个批次只消费尚未处理的资产代次，结束后保存状态和退出码。
6. 新资产回流后可启动下一批，不必等待所有耗时工具一次性全部结束。

Agent 停滞告警
   - 批次目录超过配置分钟数没有文件更新时，项目状态会标记为疑似停滞。
   - 默认只记录告警，不会自动结束、杀掉或重启 Agent；设置为 0 可关闭。
   - 这类状态通常表示模型线路、CLI 等待或网络问题，需要检查 Agent PowerShell 窗口。
