...
2. 全局设置
   ...
   - 这里主要配置：
     ...
     - 工作方式选项，例如是否等待 AssetCommander、是否等待 fscan、最多执行多少次 AI 等。
...
3. 授权范围
   ...
   - 主要目标不会自动替代授权范围，可在启动页点击“使用主要目标”快速填写。
   - 新项目不要使用 *。历史项目中的 * 仅为兼容旧配置，表示不做范围过滤，不能作为实际授权证明。
...
6. 资产汇总队列统一吸收这些新增资产，并记录资产更新轮次；
7. 自动调度器判断：
   - AssetCommander 是否完成；
   - fscan 是否完成；
   - 资产是否进入安静窗口；
8. 满足条件后再启动一次 AI 执行；
9. AI 逐次验证 Web 资产、读取结果文件、生成线索；
...
3. 资产汇总队列维护资产更新轮次、来源、安静窗口和就绪状态。
4. 达到启动条件后，自动调度器在共享 Windows Terminal 窗口中启动一次 CLI AI 执行。
5. 每次执行只处理 AI 尚未处理的资产更新轮次，结束后保存状态和退出码。
6. 新资产回流后可启动下一批，不必等待所有耗时工具一次性全部结束。
